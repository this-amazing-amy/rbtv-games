...
import { updatePlayerFEHBKeyCode, updatePlayerName, removePlayer } from '../../state/players';
...
const changeKey = player => (e) => {
  ...
  const assignKey = (evt) => {
    updatePlayerFEHBKeyCode(evt.keyCode, player);
...
export default {
  view: ({ attrs: { player } }) => m('.player.player-form', [
    ...
    }, `Key: ${String.fromCharCode(player.fehb.keyCode)}` || 'No key assigned!'),
